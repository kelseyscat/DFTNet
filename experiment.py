...
def cal_ssim(img1, img2, dynamic_range=10000):
...
class Experiment(object):
    def __init__(self, option):
        ...
        # self.generator是整个网络模型
        self.generator = Generator().to(self.device)
        self.nlayerdiscriminator = NLayerDiscriminator(input_nc=8, getIntermFeat=True).to(self.device)
    ...
    def train_on_epoch(self, n_epoch, data_loader):
        ...
        for idx, data in enumerate(data_loader):
            ...
            data[0] = interpolate(interpolate(data[0], scale_factor=1 / 16), scale_factor=16, mode='bicubic')  # 插值
            ...
            prediction = self.generator(inputs)  # 生成器是整体模型

            pred_fake = self.nlayerdiscriminator(torch.cat((prediction.detach(), inputs[2]), dim=1))  # inputs[2]指代？
    ...
    def train(self, train_dir, val_dir, patch_stride, batch_size,
        ...
        for epoch in range(start_epoch, epochs + start_epoch):
            ...
            val_error, val_ssim = self.test_on_epoch(val_loader)
            print(val_ssim)
    ...
    def test_on_epoch(self, data_loader):
        ...
        for data in data_loader:
            ...
            for i in range(len(target)):
                img1 = target[i, :, :, :].cpu().numpy().transpose(1, 2, 0)
                img2 = prediction[i, :, :, :].cpu().numpy().transpose(1, 2, 0)
                ssims = cal_ssim(img1 * 10000, img2 * 10000)
                ssim_sum += np.array(ssims).mean()
            ssim_all.update(ssim_sum / len(target))
            g_loss = F.mse_loss(prediction, target)
            epoch_error.update(g_loss.item())
        return epoch_error.avg, ssim_all.avg
    ...
    def test(self, test_dir, image_size, patch_size, num_workers=0):
        ...
        for inputs in test_loader:
            ...
            result = splicing_image(result_list, coor)  # 这么做是为了没有拼接线
