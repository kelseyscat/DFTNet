...
def sam(img1, img2):  # 光谱角相似性，img1真实的，img2预测的
    ...
    cos_theta = (inner_product / (img1_spectral_norm * img2_spectral_norm + np.finfo(np.float64).eps)).clip(min=0,
                                                                                                            max=1)  # clip将数组的值限制在最大值与最小值之间，如果大于1那就默认等于1.
    return np.mean(np.arccos(cos_theta))


def psnr(img1, img2, dynamic_range=10000):  # img1是真实的，img2是预测的。
...
def ssim(img1, img2, dynamic_range=10000):
...
def ergas(img_fake, img_real, scale=16):
